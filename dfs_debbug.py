...
class CarlaEnv:
    def __init__(self):
        ...
        self.image_counter = 0  # Counter for image updates
    ...
    def reset_environment(self):
        ...
        self.spawn_point = random.choice(self.spawn_points)  # Use random spawn point
    ...
    def process_image(self, image):
        try:
            ...
            with self.image_lock:
                ...
                self.image_timestamp = time.time()  # Update timestamp
            ...
            if DEBUG:
                print(f"[DEBUG] Image processed at {self.image_timestamp}, Total Images: {self.image_counter}")
...
def train_agent(env, agent, replay_buffer, num_episodes=1000, max_steps_per_episode=1000, batch_size=64):
    try:
        previous_image_counter = env.image_counter  # Initialize previous image counter
        ...
        for episode in range(num_episodes):
            ...
            for step in range(max_steps_per_episode):
                ...
                # Log image update
                if DEBUG:
                    print(f"[DEBUG] Episode {episode+1}, Step {step}: Image Counter = {env.image_counter}")

                # Check if a new image has been received
                assert env.image_counter > previous_image_counter, "Image was reused! Counter did not increment."
                previous_image_counter = env.image_counter  # Update for next step
                ...
                if DEBUG:
                    print(f"[DEBUG] Episode {episode+1}, Step {step}, Reward: {reward:.2f}, Total Reward: {episode_reward:.2f}")
                ...
                if done:
                    if DEBUG:
                        print(f"[DEBUG] Episode {episode+1} terminated due to {termination_reason}.")
                    break
